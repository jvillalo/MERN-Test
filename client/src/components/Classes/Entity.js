...
class Entity {
  ...
  createEntity(graph, level) {
    ...
    try {
      ...
      if (this.styleText != "") {
        estyle =
          "horizontal=1;verticalLabelPosition=bottom;verticalAlign=top;resizable=1;movable=1;shape=image;image=" +
          this.styleText;
      } else {
        estyle = "entity";
      }
      ...
      if (this.directType != "") {
        text =
          ...
          "</sup>:" +
          this.directType;
      } else text = "<b>" + this.name + "</b><sup>" + this.potency + "</sup>";
  ...
  createFeatures(graph) {
    ...
    if (this.styleText != "") {
      ...
      for (j = 0; j < this.methods.length; j++) {
        var text =
          " " +
          this.methods[j].getName() +
          "<sup>" +
          this.methods[j].durability +
          "</sup>(" +
          this.methods[j].signature +
          "){ " +
          this.methods[j].body +
          " }";
        var v1 = graph.insertVertex(
